...
Sheet::Sheet(std::string t_name, uint8_t t_strength, uint8_t t_dexterity,
    ...
    :
    m_name(t_name), m_strength(t_strength), m_dexterity(t_dexterity),
    m_constitution(t_constitution), m_inteligence(t_inteligence),
    m_wisdom(t_wisdom), m_charisma(t_charisma), m_level(t_level),
    m_maxLife(t_maxLife), m_currentLife(t_maxLife)
{
}
...
void Sheet::set_strength(short t_strength)
{
    if(t_strength > 0) m_strength = t_strength;
}

void Sheet::set_dexterity(short t_dexterity)
{
    if(t_dexterity > 0) m_dexterity = t_dexterity;
}

void Sheet::set_constitution(short t_constitution)
{
    if(t_constitution > 0) m_constitution = t_constitution;
}

void Sheet::set_inteligence(short t_inteligence)
{
    if(t_inteligence > 0) m_inteligence = t_inteligence;
}

void Sheet::set_wisdom(short t_wisdom)
{
    if(t_wisdom > 0) m_wisdom = t_wisdom;
}

void Sheet::set_charisma(short t_charisma)
{
...
// from damages.
void Sheet::set_currentLife(short t_currentLife)
{
...
short Sheet::dexterity() const
{
    return m_dexterity;
}

short Sheet::constitution() const
{
    return m_constitution;
}

short Sheet::inteligence() const
{
    return m_inteligence;
}

short Sheet::wisdom() const
{
    return m_wisdom;
}

short Sheet::charisma() const
{
    return m_charisma;
}

short Sheet::modifier(uint8_t t_atribute) const
{
...
const std::string Sheet::showStats() const
{
    std::stringstream stream;

    stream
    << "Strength: " << std::showpos << strength() << " (" << 
        modifier(strength() ) << ")\n"
    << "Dexterity: " << std::showpos << dexterity() << " (" << 
        modifier(dexterity() ) << ")\n"
    << "Constitution: " << std::showpos << constitution() << " (" << 
        modifier(constitution() ) << ")\n"
    << "Inteligence: " << std::showpos << inteligence() << " (" << 
        modifier(inteligence() ) << ")\n"
    << "Wisdom: " << std::showpos << wisdom() << " (" << 
        modifier(wisdom() ) << ")\n"
    << "Charisma: " << std::showpos << charisma() << " (" << 
        modifier(charisma() ) << ')';
